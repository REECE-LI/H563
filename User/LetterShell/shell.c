...
static const char *shellText[] = {
    [TEXT_INFO] = "\r\n"
                  ...
                  "  \\___ \\| |  | |\\___ \\   | |/ _ \\/ __| '_ \\ \r\n"
                  "  ____) | |__| |____) |  | |  __/ (__| | | |\r\n"
                  " |_____/ \\____/|_____/   |_|\\___|\\___|_| |_|\r\n"
                  "\r\n"
                  "Build:       "__DATE__
                  " "__TIME__
                  "\r\n"
...
const SHELL_KeyFunctionDef shellDefaultKeyFunctionList[] = {
    {SHELL_KEY_LF, shellEnter},         {SHELL_KEY_CR, shellEnter},
    {SHELL_KEY_TAB, shellTab},          {SHELL_KEY_BACKSPACE, shellBackspace},
    {SHELL_KEY_DELETE, shellBackspace}, {SHELL_KEY_ESC, shellAnsiStart},
};
...
void shellInit(SHELL_TypeDef *shell)
{
  ...
  shell->commandNumber =
      ((unsigned int)(&shellCommand$$Limit) - (unsigned int)(&shellCommand$$Base)) / sizeof(SHELL_CommandTypeDef);
#if SHELL_USING_VAR == 1
  shell->variableBase = (SHELL_VaribaleTypeDef *)(&shellVariable$$Base);
  shell->variableNumber =
      ((unsigned int)(&shellVariable$$Limit) - (unsigned int)(&shellVariable$$Base)) / sizeof(SHELL_VaribaleTypeDef);
#endif /** SHELL_USING_VAR == 1 */

#elif defined(__ICCARM__)
  shell->commandBase = (SHELL_CommandTypeDef *)(__section_begin("shellCommand"));
  shell->commandNumber =
      ((unsigned int)(__section_end("shellCommand")) - (unsigned int)(__section_begin("shellCommand"))) /
      sizeof(SHELL_CommandTypeDef);
#if SHELL_USING_VAR == 1
  shell->variableBase = (SHELL_VaribaleTypeDef *)(__section_begin("shellVariable"));
  shell->variableNumber =
      ((unsigned int)(__section_end("shellVariable")) - (unsigned int)(__section_begin("shellVariable"))) /
  ...
  shell->commandNumber =
      ((unsigned int)(&_shell_command_end) - (unsigned int)(&_shell_command_start)) / sizeof(SHELL_CommandTypeDef);
#if SHELL_USING_VAR == 1
  ...
  shell->variableNumber =
      ((unsigned int)(&_shell_variable_end) - (unsigned int)(&_shell_variable_start)) / sizeof(SHELL_VaribaleTypeDef);
#endif /** SHELL_USING_VAR == 1 */
#else
...
static void shellHistory(SHELL_TypeDef *shell, unsigned char dir)
{
  ...
  if (dir == 0)
  {
    if (shell->historyOffset-- <=
        -((shell->historyCount > shell->historyFlag) ? shell->historyCount : shell->historyFlag))
    {
      shell->historyOffset = -((shell->historyCount > shell->historyFlag) ? shell->historyCount : shell->historyFlag);
    }
  }
  else if (dir == 1)
  {
  ...
  {
    if ((shell->length = shellStringCopy(
             shell->buffer, shell->history[(shell->historyFlag + SHELL_HISTORY_MAX_NUMBER + shell->historyOffset) %
...
static void shellEnter(SHELL_TypeDef *shell)
{
  ...
  *(shell->buffer + shell->length++) = 0;
  ...
  {
    if ((quotes != 0 || (*(shell->buffer + i) != ' ' && *(shell->buffer + i) != '\t' && *(shell->buffer + i) != ',')) &&
        *(shell->buffer + i) != 0)
    {
      if (*(shell->buffer + i) == '\"')
      {
  ...
  for (unsigned short i = 0; i < shell->commandNumber; i++)
  {
    if (strcmp((const char *)shell->param[0], (base + i)->name) == 0)
    {
      ...
      shell->status.isActive = 1;
#if SHELL_AUTO_PRASE == 0
      returnValue = ((int (*)(uint8_t, char**))base[i].function)
                  (paramCount, shell->param);
...
static void shellTab(SHELL_TypeDef *shell)
{
  ...
  if (shell->length != 0)
  {
    ...
    {
      if (shellStringCompare(shell->buffer, (char *)(base + i)->name) == shell->length)
      {
        if (matchNum != 0)
        {
          if (matchNum == 1)
          ...
          length = shellStringCompare((char *)(base + lastMatchIndex)->name, (char *)(base + i)->name);
    ...
    if (matchNum != 0)
    {
      shell->length = shellStringCopy(shell->buffer, (char *)(base + lastMatchIndex)->name);
    }
    if (matchNum > 1)
    {
      shellDisplayItem(shell, lastMatchIndex);
 ...
 * @param shell shell对象
 */
static void shellAnsiStart(SHELL_TypeDef *shell) { shell->status.inputMode = SHELL_ANSI_ESC; }
...
void shellHandler(SHELL_TypeDef *shell, char data)
{
  ...
  if (shell->status.inputMode == SHELL_IN_NORMAL)
  {
    ...
    if (keyDefFind == 0)
    {
      for (short i = 0; i < sizeof(shellDefaultKeyFunctionList) / sizeof(SHELL_KeyFunctionDef); i++)
      {
...
uint8_t shellRxBuffer[SHELL_COMMAND_MAX_LENGTH];
 ...
 */
void ShellWrite(char data) { HAL_UART_Transmit(&huart1, (uint8_t *)&data, 1, 0xFFFF); }
